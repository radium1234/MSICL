...
class Dataset:
    ...
    def _data_processing(self):
        ...
        id=self.inter_feat["item_id"]
        self._remap_ID_all()
        newid=self.inter_feat["item_id"]
        ni=dict(zip(newid,id))
        in_=dict(zip(id,newid))
        with open(self.dataset_path+"/newid-id.json","w+") as f:
            json.dump(ni,fp=f)
        with open(self.dataset_path+"/id-newid.json","w+") as f:
            json.dump(in_,fp=f)
    ...
    def _data_filtering(self):
        ...
        self._filter_by_modal()
        self._reset_index()

    def readImageFeatures(self, path):
        f = open(path, 'rb')
        k = 0
        d = []
        while True:
    ...
    def imagelist(self):
        d = self.readImageFeatures(self.dataset_path + "/../" + self.dataset_name + '.b')
        np.save(self.dataset_path + '/imagelist.npy', d)

    def _filter_by_modal(self):
        if not os.path.exists(self.dataset_path + "/imagelist.npy"):
            self.imagelist()
        a = np.load(self.dataset_path + "/imagelist.npy")
        self.inter_feat = self.inter_feat[self.inter_feat["item_id"].isin(a)]
